...
func (h *PublicHandler) Rankings(w http.ResponseWriter, r *http.Request) {
	...
	i := 1
	for rows.Next() {
		var fn, ln, c string; var total int
		if err := rows.Scan(&fn, &ln, &c, &total); err != nil { http.Error(w, "db error", http.StatusInternalServerError); return }
		out = append(out, row{Position: i, Username: fn+" "+ln, City: c, Votes: total}); i++
	}
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(out)
}
